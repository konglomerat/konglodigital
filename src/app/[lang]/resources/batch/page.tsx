...
export default function BatchResourcePage() {
      ...
      try {
        ...
        if (!newStream) {
          throw lastError instanceof Error
            ? lastError
            : new Error(tx("Unable to access camera."));
        }
        ...
        setCameraError(
          error instanceof Error
            ? error.message
            : tx("Unable to access camera."),
        );
        setNeedsPermission(true);
      }
    },
    [selectedDeviceId, stopStream, tx],
  );
  ...
  const handleCapture = async () => {
    ...
    try {
      ...
      if (!videoRef.current) {
        ...
        setFormError(tx("Camera not ready."));
        return;
      }
      if (!videoReady) {
        console.log("Batch capture: video not ready");
        setFormError(
          tx("Camera is still loading. Please try again in a moment."),
        );
        return;
      }
      if (videoRef.current.readyState < 2) {
        console.log("Batch capture: readyState", videoRef.current.readyState);
        setFormError(tx("Camera stream not ready yet."));
        return;
      }
      if (isCapturing) {
      ...
      if (!file) {
        ...
        setFormError(tx("Unable to capture photo."));
        return;
      }
      let previewUrl = "";
      try {
      ...
      setFormError(
        error instanceof Error
          ? error.message
          : tx("Capture failed unexpectedly."),
      );
    } finally {
  ...
  const handleSend = () => {
    ...
    if (!canSend || !location) {
      setFormError(
        photos.length === 0
          ? tx("Capture at least one photo before sending.")
          : tx("Map location not ready yet."),
      );
      return;
    }
        ...
        >
          {devices.map((device, index) => (
            <option key={device.deviceId} value={device.deviceId}>
              {device.label || `${tx("Camera")} ${index + 1}`}
            </option>
          ))}
        </select>
      </div>
    );
  }, [devices, selectedDeviceId, tx]);
        ...
        <div className="absolute top-0 left-0 right-0 z-10 flex flex-wrap items-start gap-2 px-2 py-2 pt-2">
          ...
          {jobs.length > 0 ? (
            ...
            <p className="text-xs text-zinc-500 py-3 grow-1 text-right">
              {tx("Recent uploads will appear here")}
            </p>
          )}
        </div>
          ...
          <div>
            {photos.length > 0 ? (
              ...
              <p className="text-xs text-zinc-400">
                {isCapturing
                  ? tx("Processing photo...")
                  : tx("Tap the shutter to capture photos")}
              </p>
            )}
          </div>
        </div>
