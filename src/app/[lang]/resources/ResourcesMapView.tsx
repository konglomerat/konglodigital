...
export default function ResourcesMapView({
  ...
  const emitVisibleResourceIds = useCallback(
    (map: Map) => {
      ...
      if (!bounds) {
        if (lastVisibleResourceIdsRef.current.length === 0) {
          return;
        }
        lastVisibleResourceIdsRef.current = [];
        onVisibleResourceIdsChange([]);
        return;
      }
      ...
      if (haveSameResourceIds(lastVisibleResourceIdsRef.current, visibleIds)) {
        return;
      }

      lastVisibleResourceIdsRef.current = visibleIds;
      onVisibleResourceIdsChange(visibleIds);
    },
    [haveSameResourceIds, locations, onVisibleResourceIdsChange],
  );

  useEffect(() => {
    const token = process.env.NEXT_PUBLIC_MAPBOX_TOKEN;
    if (!token) {
      setMapboxError(missingMapboxTokenMessage);
      return;
    }
